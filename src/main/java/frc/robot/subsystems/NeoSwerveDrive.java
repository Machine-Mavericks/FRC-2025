...
public class NeoSwerveDrive extends SubsystemBase {
    ...
    // Swerve module states - contains target speed(m/s) and angle for each swerve
    // module
    ...
    public NeoSwerveDrive() {
        ...
        // units - in degrees
        // note: if +ve value when wheels are aligned straight, then decrease offset by
        // that value
    ...
    // seed the encoder value of steering motors based on reported CANcoder
    // positions
    ...
    public void FieldDrive(ChassisSpeeds speed, boolean Park) {
        ...
        ChassisSpeeds newSpeed = ChassisSpeeds.fromFieldRelativeSpeeds(speed,
                Rotation2d.fromDegrees(RobotContainer.gyro.getYawAngle()));
    ...
    public void RobotDrive(ChassisSpeeds speed, boolean Park) {
        ...
        if (Park) {
            // Note that the LF and RR wheels are both set to the same angle. Same for the
            // RF and LR wheels
            m_states[0].angle = m_ParkAngleLF;
            m_states[0].speedMetersPerSecond = 0;
            m_states[1].angle = m_ParkAngleRF;
            m_states[1].speedMetersPerSecond = 0;
            m_states[2].angle = m_ParkAngleRF;
            m_states[2].speedMetersPerSecond = 0;
            m_states[3].angle = m_ParkAngleLF;
            m_states[3].speedMetersPerSecond = 0;
        }

        // if desired speed of a swerve(s) exceed maximum possible, then reduce all
        // speeds while maintaining ratio
        ...
        m_LFSteerMotor.getClosedLoopController().setReference(m_LFSteerMotor_Ref, ControlType.kPosition,
        ...
        m_RFSteerMotor.getClosedLoopController().setReference(m_RFSteerMotor_Ref, ControlType.kPosition,
                ClosedLoopSlot.kSlot0);
        ...
        m_LRSteerMotor.getClosedLoopController().setReference(m_LRSteerMotor_Ref, ControlType.kPosition,
                ClosedLoopSlot.kSlot0);
        ...
        m_RRSteerMotor.getClosedLoopController().setReference(m_RRSteerMotor_Ref, ControlType.kPosition,
                ClosedLoopSlot.kSlot0);
        ...
        m_LFDriveMotor.getClosedLoopController().setReference(m_LFDriveMotor_Ref, ControlType.kVelocity,
                ClosedLoopSlot.kSlot0);
        m_RFDriveMotor_Ref = m_states[1].speedMetersPerSecond * RFDriveDir * MPS_TO_WHEELRPM;
        m_RFDriveMotor.getClosedLoopController().setReference(m_RFDriveMotor_Ref, ControlType.kVelocity,
                ClosedLoopSlot.kSlot0);
        m_LRDriveMotor_Ref = m_states[2].speedMetersPerSecond * LRDriveDir * MPS_TO_WHEELRPM;
        m_LRDriveMotor.getClosedLoopController().setReference(m_LRDriveMotor_Ref, ControlType.kVelocity,
                ClosedLoopSlot.kSlot0);
        m_RRDriveMotor_Ref = m_states[3].speedMetersPerSecond * RRDriveDir * MPS_TO_WHEELRPM;
        m_RRDriveMotor.getClosedLoopController().setReference(m_RRDriveMotor_Ref, ControlType.kVelocity,
                ClosedLoopSlot.kSlot0);
    ...
    public SwerveModulePosition[] GetSwerveDistances() {
        ...
        // populate distance(m) and angle for LF swerve (note: rps_to_mps same as
        // r_to_m)
    ...
    private void initializeShuffleboard() {
        ...
        Tab.add("Reset Swerve", new InstantCommand(() -> {
            ResetSteerEncoders();
        }))
                .withPosition(4, 0)
                .withSize(2, 1);
