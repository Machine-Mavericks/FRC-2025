...
public class ManualDrive extends Command {
    ...
    // PID controller used to counteract rotational drift due to misalignment of
    // wheels
    ...
    public void execute() {
        ...
        // made the dead zones a circle instead of a square, old equation commented
        // below
        ...
        // Auto-snap to reef functionality removed (was dependent on intake sensor)

        // --------- Correct robot angle for gyro angle wander --------
        if (false && omega == 0.0 && !RobotContainer.driverOp.back().getAsBoolean()) {// was EnableDriftCorrection
            if (m_pidDelay > 0)
                m_pidDelay--;
            else {
            ...
            // there is rotational input, or gyro reset was pressed, set target to null so
            // it's properly reset next time
            m_PIDTarget = null;
            m_pidDelay = 10;
        }
        ...
        // very small non-zero robot rotation. This resutls in wheels always resetting
        // to
        ...
        // Since the drive was shifted to closed loop (i.e. requested velocities),
        // change joystick input max values
        // to MAX_SPEED values.
    ...
    // This method to return true only when command is to finish. Otherwise return
    // false
    @Override
